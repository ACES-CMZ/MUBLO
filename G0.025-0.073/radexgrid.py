...
for ii, tem in enumerate((10, 15, 25, 50)):
    ...
    pl.contour(np.log10(out_crds[1][:,:,temind]), np.log10(out_crds[0][:,:,temind]), ratiocube_fine[:,:,temind], levels=[4.26,4.87], colors=['r']*2)
    pl.contour(np.log10(out_crds[1][:,:,temind]), np.log10(out_crds[0][:,:,temind]), so32tbcube_fine[:,:,temind], levels=[1.7,1.8], colors=['b']*2)
    pl.contour(np.log10(out_crds[1][:,:,temind]), np.log10(out_crds[0][:,:,temind]), so21tbcube_fine[:,:,temind], levels=[0.36, 0.40], colors=['c']*2)
...
pl.axhline(4.56, linestyle='--', color='k')
pl.legend()
